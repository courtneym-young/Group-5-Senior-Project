  ...
  const fetchBusiness = async () => {
    ...
    try {
      ...
      if (!result.data) {
        ...
        if (
          resolvedBusinessOwnerPosts &&
          typeof resolvedBusinessOwnerPosts === "object" &&
          "then" in resolvedBusinessOwnerPosts
        ) {
        ...
        if (
          resolvedBusinessProducts &&
          typeof resolvedBusinessProducts === "object" &&
          "then" in resolvedBusinessProducts
        ) {
        ...
        if (
          resolvedReviews &&
          typeof resolvedReviews === "object" &&
          "then" in resolvedReviews
        ) {
        ...
        if (
          resolvedSubscribers &&
          typeof resolvedSubscribers === "object" &&
          "then" in resolvedSubscribers
        ) {
        ...
        if (Array.isArray(resolvedReviews)) {
          resolvedReviews = await Promise.all(
            resolvedReviews.map(async (review) => {
              if (
                review.user &&
        ...
        if (Array.isArray(resolvedBusinessOwnerPosts)) {
          resolvedBusinessOwnerPosts = await Promise.all(
            resolvedBusinessOwnerPosts.map(async (post) => {
              if (
                post.user &&
        ...
        if (Array.isArray(resolvedSubscribers)) {
          resolvedSubscribers = await Promise.all(
            resolvedSubscribers.map(async (subscriber) => {
              if (
                subscriber.user &&
                typeof subscriber.user === "object" &&
                "then" in subscriber.user
              ) {
      ...
      setError(
        `Failed to fetch business: ${error instanceof Error ? error.message : String(error)}`
      );
      setBusiness(null);
    } finally {
      setLoading(false);
